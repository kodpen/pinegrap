...
if (!$_POST) {
    ...
    echo pg_page_shell([
        ...
        'icon'=>'setting',
        'heading'=>lang('MailChimp Settings'),
        'cancel'=>['enable'=>'true','title'=>lang('Return to Settings'),'url'=>'settings.php'],
        'auto_main'=>false,
    ]);
...
// Otherwise the form has been submitted so process it.
    ...
    if ($mailchimp) {
        ...
        $form->validate_required_field('mailchimp_key', lang(array('string'=>'{var:1} is required.','vars'=>lang('API Key') )) );
        $form->validate_required_field('mailchimp_list_id', lang(array('string'=>'{var:1} is required.','vars'=>lang('List ID') )) );

        $form->validate_required_field('mailchimp_store_id', lang(array('string'=>'{var:1} is required.','vars'=>lang('Store ID') )) );
        ...
        if ($response['status'] == 'error') {
            $form->mark_error('mailchimp_key', lang('Sorry, the API key is not valid') . '. ' .
                h($response['message']));
            go($_SERVER['PHP_SELF']);
        }
        ...
        if ($response['status'] == 'error') {
            $form->mark_error('mailchimp_list_id', lang('Sorry, the List ID is not valid') . '. ' .
                h($response['message']));
            go($_SERVER['PHP_SELF']);
        }
        ...
        if ($response['status'] == 'error' and $response['mailchimp_response']['status'] != 404) {
            $form->mark_error('mailchimp_store_id', lang('Sorry, the Store ID is not valid') . '. ' .
                h($response['message']));
            go($_SERVER['PHP_SELF']);
        }
        ...
        if ($response['status'] == 'error' and $response['mailchimp_response']['status'] == 404) {
            ...
            if ($response['status'] == 'error') {
                $form->mark_error('mailchimp_store_id', lang('Sorry, the store could not be created') . '. ' .
                    h($response['message']));
                go($_SERVER['PHP_SELF']);
            }
        ...
        if (!$mailchimp_automation != $store['is_syncing']) {
            ...
            if ($response['status'] == 'error') {
                $form->mark_error('mailchimp_automation', lang('Sorry, automation could not be updated') . '. ' .
                    h($response['message']));
                go($_SERVER['PHP_SELF']);
            }
        }
    ...
    $message = lang('The MailChimp settings have been saved.');
