...
if (!$_POST) {
    ...
    echo pg_page_shell([
        'title'=> lang('Create Offer Rule'),
        'extra classes'=>'products',
        'icon'=>'store',
        'heading'=>lang('Create Offer Rule'),
...
// Otherwise the form was just submitted so process it.
    ...
    $form_get_name = $form->get('name');
    log_activity( lang(array('string'=>'Offer rule ({var:1}) was created.','vars'=>$form_get_name)) );
    $form->remove();
    $form_view_offer_rules = new liveform('view_offer_rules');
    $form_view_offer_rules->add_notice(lang('The offer rule has been created.'));
